...
- **PCE tables**: `tests/test_pce_table_epub.py` regenerates `pce_table.rs` from the EPUB and asserts the committed file matches byte for byte; it skips when `resources/` is absent (gitignored, copyrighted).
