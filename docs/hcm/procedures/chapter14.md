...
2. ~~**Major merge LOS return value**~~ **Fixed in 0.3.0.** `determine_los` for `MajorMerge` under capacity used to set `self.los = None` (correct — HCM defines no LOS) while *returning* `LevelOfService::E`, so a caller reading the return value got a fabricated letter while a caller reading the field got the truth, and `run_analysis` propagated the same E. Both now return `Option<LevelOfService>` and yield `None` in that case, which is a breaking change recorded in the CHANGELOG; the PyO3 `run_analysis` returns `None` likewise. Pinned by `major_merge_under_capacity_has_no_seventh_edition_los`. Edition 7.1 closes the hole from the other side, since Exhibit 14-2 extends its criteria to major merges and diverges, so the 7.1 path always yields a letter (see `chapter13-14-v7-1.md`).
...
4. **Mountainous PCE (VERIFY-HCM in `fhv_for`)**: the Equation 14-1 note says the adjustment factors "are the same as those used in Chapter 12", and Chapter 12 tabulates level and rolling only while directing mountainous terrain to the Chapter 25/26 mixed-flow model. Chapter 14 lists mountainous among its required terrain inputs (both for the ramp and for the freeway) but never supplies a PCE for it, so `E_T = 5.0` stands in for a quantity no exhibit defines — same flag as chapter13, and inconsistent with the 2.5 in `basicfreeways` and the 3.0 in `freeway_facilities`. Not made an error because the refusal would have to travel out through this module's infallible public surface; keep-vs-error is the open decision in `VERIFICATION.md` (Chapter 12-14 item 1). No published example problem exercises a mountainous ramp junction.
...
6. ~~**8-lane P_FM applicability**~~ **Fixed in 0.3.0.** The Exhibit 14-8 8-lane equations are transcribed with the v_F/S_FR ≤ 72 split, and used to let the result leave [0, 1]: `0.2178 − 0.000125·v_R` goes negative for v_R above 0.2178/0.000125 ≈ 1,742 pc/h, which put a negative proportion in Lanes 1 and 2 and a negative density downstream. Both Exhibit 14-8 eight-lane forms and the Exhibit 14-9 base form now pass through `clamp_proportion`, carrying a VERIFY-HCM note that a clamp means the input is outside the range the regression was fitted on and the result should be read as an extrapolation. Pinned by `eight_lane_pfm_stays_a_proportion_at_high_ramp_demand`.
