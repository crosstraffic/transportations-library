...
1. **Mountainous PCE (VERIFY-HCM in `calculate_fhv`)**: Step 1 states that f_HV "is taken from Chapter 12", so this chapter inherits Chapter 12's gap rather than filling it — Exhibit 12-25 tabulates level and rolling only, and the Chapter 12 text directs mountainous terrain to the Chapter 25/26 mixed-flow model. The code's `E_T = 5.0` therefore stands in for a quantity no exhibit defines. It is not replaced by the mixed-flow model, which needs a grade percent and length and returns speeds rather than a PCE, and it is not made an error because the refusal would have to travel out through `determine_demand_flow` and the rest of this module's infallible public surface. The four library stand-in sites span three distinct values (2.5 in `basicfreeways`, 3.0 in `freeway_facilities`, 5.0 here and in `merge_diverge`); keep-vs-error is the open decision in `VERIFICATION.md` (Chapter 12-14 item 1). No published example problem exercises mountainous weaving.
