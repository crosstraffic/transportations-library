...
| Step 9 — LOS | Exhibit 23-10; Eqs. 23-51/23-52 | `Interchange::step_9_od_ett_and_los`, `od_path`, `los_signalized_interchange_od` | `ramp_terminals.rs`, `exhibits.rs` | per-O-D summed control delay + EDTT (s/veh); the v/c>1 and R_Q>1 flags apply per O-D only, never to the interchange aggregate | `OdResult.ett_s`, `.los`; `self.interchange_ett_s`, `.interchange_los` |
...
`InterchangeMovement` is a composition rather than a closed list: an `InterchangeApproach` (`Eastbound`/`Westbound`/`Northbound`/`Southbound`), a `MovementPosition` (`External`/`Internal`/`Ramp`/`RampTwo`), and a `MovementTurn` (`Left`/`Through`/`Right`/`ThroughRight`). It serialises to the concatenation of the three names, so the ten movements of a diamond skeleton are the strings `EbExtThrough`, `EbIntThrough`, `EbIntLeft`, `WbExtThrough`, `WbIntThrough`, `WbIntLeft`, `NbRampLeft`, `NbRampRight`, `SbRampLeft`, `SbRampRight` — the same strings the earlier closed enum produced, which is the whole of the compatibility surface (there are no aliases and no shim; the legacy names *are* compositions, pinned by `test_legacy_movement_names`). `Debug` and `Display` print the same string, so any caller that formatted a movement into a message or a Python-facing name is unaffected. The `movements` module exposes each composition as a constant (`movements::EB_EXT_THROUGH`) for `match` arms and lane group construction. `MovementPosition::RampTwo` covers the AB and B-4Q parclos, where both loop ramps are fed from one freeway direction and the same compass approach therefore appears at both terminals (the `_ii` columns of Exhibits 34-165, 34-166, and 34-168).

The compositions the diamond skeleton never reached are what the parclo family needs: an external left (`EbExtLeft`, the arterial left onto a parclo A loop, "EXT-L" in Exhibit 34-22), an internal shared through-and-right (`EbIntThroughRight`, "INT-TH&R"), and an exclusive external right (`EbExtRight`, "EXT-R" in Exhibit 34-34). Validation status across the family: the diamond is pinned by Chapter 34 Example Problems 1, 3, and 4, the DDI by Examples 5 and 6, and the Parclo A-2Q by Example 2. The remaining five parclos and the SPUI are **structurally supported and unvalidated** — their routing and lane group composition come from the same worksheets, and `test_every_form_runs_the_pipeline` and `test_every_form_routes_every_od` exercise each of the nine forms end to end on a synthetic configuration, but Chapter 34 publishes no Part B worked example that would pin their numbers.
...
`step_8_control_delay` dispatches per lane group on `LaneGroupControl`: `FreeFlow` groups carry zero control delay (Chapter 23 Step 6 text); `YieldControlled` groups use the Chapter 22 roundabout control-delay procedure (`common::delay::control_delay_roundabout`, Equation 22-17) per the Chapter 23 Step 8 text and Chapter 34 Example Problem 6's explicit instruction to do so, though this is flagged `VERIFY-HCM` since Exhibit 34-70's published delays are not reproducible from Equation 22-17 (see Deviations); `Signalized` groups use the standard Chapter 19 uniform delay (`uniform_delay`, Equation 19-19, with `progression_factor` for P = R_p g/C), incremental delay evaluated **on the lane group capacity** (`incremental_delay_signalized` called with the Step 7 `capacity`, which is what Equation 19-26 defines c_A as; the Chapter 34 worksheets' per-lane convention was followed until 0.3.1 and is now treated as a book defect, see Deviations), and initial-queue delay (`initial_queue_delay`). Back-of-queue and queue storage ratio reuse the Chapter 19/31 building blocks directly (`first_term_back_of_queue`, `second_term_back_of_queue`, `average_vehicle_spacing`, `queue_storage_ratio_eq`). Step 9's `od_path(OdMovement) -> Vec<InterchangeMovement>` gives each of the fourteen O-D letters the lane groups it traverses, derived from the form's own Chapter 34 worksheet rather than tabulated per form: `od_journey(form, m)` pushes a unit demand on the O-D through `turning_movements_from_od` and reads back the turning movements it lands in, and `Interchange::resolve_movement` maps each of those onto a lane group the interchange actually has (an internal right folding into an internal through-and-right group, an external turn folding into the external through group when the direction is flagged as sharing, and resolving to nothing otherwise, which is the free-flow bypass case a DDI right turn takes), and `step_9_od_ett_and_los` sums each path's control delays, adds the Equation 23-50 extra-distance travel time (`extra_distance_travel_time`, EDTT — negative for right turns per the Exhibit 23-8 sign convention), looks up LOS via `los_signalized_interchange_od` (Exhibit 23-10, forcing a stricter LOS whenever any traversed lane group has v/c or queue-storage-ratio exceeding 1.0), and aggregates the demand-weighted interchange-level ETT and LOS (Equations 23-51/23-52). Since 0.3.1 the interchange-level lookup passes `false` for both flags: Exhibit 23-10's "automatically LOS F" rule applies to an individual O-D, and Chapter 23 Step 9 grades the interchange from the demand-weighted ETT alone, explicitly anticipating a failing O-D being masked at the interchange level and directing the analyst to report the poorest O-D as context rather than to force the aggregate letter down. Propagating the flags upward had graded any interchange with one oversaturated lane group as F regardless of its ETT.
